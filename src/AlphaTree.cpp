...
template<class Pixel>
void AlphaNode<Pixel>::add(AlphaNode* q)
{
    this->area += q->area;
...
template<class Pixel>
void AlphaNode<Pixel>::add(Pixel pix_val)
{
    this->area++;
...
template<class Pixel>
void AlphaNode<Pixel>::copy(AlphaNode* q)
{
    this->area = q->area;
...
template<class Pixel>
void AlphaNode<Pixel>::connect_to_parent(AlphaNode* pPar, Imgidx iPar)
{
    this->parentidx = iPar;
    pPar->add(this);
}
...
template<class Pixel>
void AlphaNode<Pixel>::print(AlphaNode* node)
{
    double val;
    ...
    if (sizeof(Pixel) > 2)
    {
        ...
        ,(int)this->area
        ,(double)log2(val + 1)
        ,(int)log2((double)(this->minPix + 1))
        ,(int)log2((double)(this->maxPix + 1))
        ,(int)this->rootidx
        ,(int)this->parentidx);			
    }
    else
...
template<class Pixel>
void AlphaNode<Pixel>::print(AlphaNode* node, int heading)
{
...
template<class Pixel>
void RankItem<Pixel>::operator=(const RankItem& item)
{
...
template<class Pixel>
Imgidx RankItem<Pixel>::get_pidx0(Imgidx connectivity)
{
    if (connectivity == 4)
...
template<class Pixel>
Imgidx RankItem<Pixel>::get_pidx1(Imgidx width, Imgidx connectivity)
{
    if (connectivity == 4)
...
void AlphaTree<Pixel>::getAlgorithmDescriptionFromCode(char* out, int algorithmCode)
{
    ...
    switch (algorithmCode)
	{
		...
		case(FLOOD_HEAPQUEUE_CACHE):
			...
			break;
	    case(FLOOD_HEAPQUEUE_NAIVE):
			strcpy(out,"Flood using quad naive heap queue (no delayed pop) (Wilkinson)");
            break;
		case(FLOOD_TRIE):
		...
		case(FLOOD_HIERARHEAPQUEUE):
			...
			break;
		case(FLOOD_LADDERQUEUE):
			strcpy(out,"Flood using hierarchical heap queue (Tang)");
			break;
		default:
...
{
    if (strcmp(algorithmName, UNIONFIND_NUM) == 0)
        return UNIONFIND;
    if (strcmp(algorithmName, UNIONFIND_COMMAND) == 0)
        return UNIONFIND;
    else if (strcmp(algorithmName, UNIONFIND_COMMAND_ALT) == 0)          
        return UNIONFIND;
    else if (strcmp(algorithmName, FLOOD_HIERARQUEUE_CACHE_NUM) == 0)
    ...
    else if (strcmp(algorithmName, FLOOD_HIERARQUEUE_NUM) == 0)
        return FLOOD_HIERARQUEUE;
    else if (strcmp(algorithmName, FLOOD_TRIE_CACHE_NUM) == 0)
        ...
        return FLOOD_TRIE;
    else if (strcmp(algorithmName, FLOOD_TRIE_NUM) == 0)
        return FLOOD_TRIE;
    else if (strcmp(algorithmName, FLOOD_HEAPQUEUE_CACHE_NUM) == 0)
...
void AlphaTree<Pixel>::BuildAlphaTree(Pixel *img, int height_in, int width_in, int channel_in, int connectivity_in, const char* algorithmName, int numthreads, int tse, double fparam1, double fparam2, int iparam1)
{
    ...
    switch (algorithmCode)
    {
        ...
        case(FLOOD_HIERARQUEUE_PAR):				Flood_Hierarqueue_par(img, numthreads);                    break;
        ...
        case(FLOOD_HEAPQUEUE_NAIVE): 			    Flood_HeapQueue_Naive(img);										break;
        ...
        case(FLOOD_TRIE_HYPERGRAPH):				Flood_Trie_Hypergraph(img);                 			        break;
        ...
        case(FLOOD_LADDERQUEUE):                    Flood_LadderQueue(img, iparam1);                                break;
...
template<class Pixel>
void AlphaTree<Pixel>::AlphaFilter(double *outimg, double alpha)
{
    Imgidx i, imgsize;
...
template<class Pixel>
void AlphaTree<Pixel>::AreaFilter(double *outimg, double area)
{
    Imgidx i, imgsize;
    Imgidx iarea;
    AlphaNode<Pixel> *pNode;
...
template<class Pixel>
void AlphaTree<Pixel>::print_tree()
{
    for (int i = 0; i < maxSize; i++)
        if (node[i].area) node[i].print(node);
}

template<class Pixel>
Pixel AlphaTree<Pixel>::abs_diff(Pixel p, Pixel q)
{
    if (p > q)
        return p - q;
    else
...
template <class Pixel>
Pixel AlphaTree<Pixel>::compute_dimg(double* dimg, Pixel* img)
{
    using Value = double;
...
template<class Pixel>
Pixel AlphaTree<Pixel>::compute_dimg1(Pixel* dimg, Imgidx* dhist, Pixel* img)
{
    Imgidx dimgidx, imgidx, stride_w = width, i, j;
    Pixel maxdiff = 0;
...
template<class Pixel>
void AlphaTree<Pixel>::compute_dimg(Pixel* dimg, Imgidx* dhist, Pixel* img, double a)
{
    Imgidx dimgidx, imgidx, stride_w = width, i, j;
    int hidx;
    imgidx = dimgidx = 0;
...
template<class Pixel>
Pixel AlphaTree<Pixel>::compute_dimg(Pixel* dimg, Imgidx* dhist, Pixel* img)
{
    Imgidx dimgidx, imgidx, stride_w = width, i, j;
    Pixel dmax = 0;
...
template<class Pixel>
double AlphaTree<Pixel>::compute_dimg(double* dimg, Imgidx* dhist, Pixel* img)
{
    Imgidx dimgidx, imgidx, stride_w = width, i, j;
    Pixel d;
    double dmax = 0;
...
template<class Pixel>
void AlphaTree<Pixel>::set_isAvailable(_uint8* isAvailable)
{
    _int32 i, j, k;
...
template<class Pixel>
_uint8 AlphaTree<Pixel>::is_available(_uint8 isAvailable, _uint8 iNeighbour)
{
    return	(isAvailable >> iNeighbour) & 1;
}

template<class Pixel>
void AlphaTree<Pixel>::set_field(_uint8* arr, Imgidx idx, _uint8 in)
{
    arr[idx] = in;
}

template<class Pixel>
_uint8 AlphaTree<Pixel>::get_field(_uint8* arr, Imgidx idx)
{
    return arr[idx];
}

template<class Pixel>
void AlphaTree<Pixel>::connectPix2Node(Imgidx pidx, Pixel pix_val, Imgidx iNode, Pixel level)
{
    AlphaNode<Pixel>* pNode;
    pNode = node + iNode;
...
template<class Pixel>
void AlphaTree<Pixel>::connectPix2Node(Imgidx pidx, Pixel pix_val, Imgidx iNode)
{
...
template<class Pixel>
void AlphaTree<Pixel>::connectPix2Node0(Imgidx pidx, Pixel pix_val, Imgidx iNode, Pixel level)
{
    AlphaNode<Pixel>* pNode;
    pNode = node + iNode;
    parentAry[pidx] = iNode;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::NewAlphaNode()
{
    if (curSize == maxSize)
    {
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::NewAlphaNode(Pixel level, AlphaNode<Pixel> *pCopy)
{
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::NewAlphaNode1(double level, AlphaNode<Pixel> *pCopy)
{
    AlphaNode<Pixel> *pNew = node + curSize;
...
template<class Pixel>
_uint8 AlphaTree<Pixel>::is_visited(_uint8* isVisited, Imgidx p)
{
    return isVisited[p];
}

template<class Pixel>
void AlphaTree<Pixel>::visit(_uint8* isVisited, Imgidx p)
{
    isVisited[p] = 1;
}
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::TreeSizeEstimation(Imgidx *dhist, _int64 numlevels, Imgidx imgsize, Imgidx nredges, double m, Imgidx reserve)
{
    nrmsd = 0;
    for (_int64 p = 0; p < numlevels; p++)
...
template<class Pixel>
void AlphaTree<Pixel>::remove_redundant_node(Imgidx& prev_top, Imgidx& stack_top)
{
...
template <class Pixel>
void AlphaTree<Pixel>::Flood_HierarQueue(Pixel* img, int tse)
{
    ...
    node[rootidx].parentidx = NULLINDEX;
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HeapQueue(Pixel* img)
{
...
FLOOD_END:
    node[stack_top].parentidx = NULLINDEX;
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HeapQueue_Naive(Pixel* img)
{
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HeapQueue_Cache(Pixel* img)
{
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HierarQueue_Cache(Pixel* img)
{
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_LadderQueue(Pixel* img, int thres)
{
    LadderQueue *queue;
    ...
    while (1)
    {
        while (queue->getTopAlpha() <= current_level) //flood all levels below current_level
        {
            ...
            {
                q = p << 1;
                if (is_available(isAv, 0) && !isVisited[p + width])	queue->enqueue(p + width, dimg[q]);
                if (is_available(isAv, 1) && !isVisited[p + 1])		queue->enqueue(p + 1, dimg[q + 1]);
                if (is_available(isAv, 2) && !isVisited[p - 1])		queue->enqueue(p - 1, dimg[q - 1]);
                if (is_available(isAv, 3) && !isVisited[p - width])	queue->enqueue(p - width, dimg[q - (width << 1)]);
            }
            else if (connectivity == 8)
            {
                Imgidx width4 = width << 2;
                q = p << 2;
                if (is_available(isAv, 0) && !isVisited[p + width])     queue->enqueue(p + width, dimg[q]);
                if (is_available(isAv, 1) && !isVisited[p + width + 1]) queue->enqueue(p + width + 1, dimg[q + 1]);
                if (is_available(isAv, 2) && !isVisited[p + 1]) 		queue->enqueue(p + 1, dimg[q + 2]);
                if (is_available(isAv, 3) && !isVisited[p - width + 1]) queue->enqueue(p - width + 1, dimg[q + 3]);
                if (is_available(isAv, 4) && !isVisited[p - width]) 	queue->enqueue(p - width, dimg[q - width4]);
                if (is_available(isAv, 5) && !isVisited[p - width - 1]) queue->enqueue(p - width - 1, dimg[q - width4 - 3]);
                if (is_available(isAv, 6) && !isVisited[p - 1]) 		queue->enqueue(p - 1, dimg[q - 2]);
                if (is_available(isAv, 7) && !isVisited[p + width - 1]) queue->enqueue(p + width - 1, dimg[q + width4 - 1]);
            }
            else
            {
                //?
...
template<class Pixel>
int AlphaTree<Pixel>::get_bitdepth(_uint64 num)
{
    int ret = 0;
    while(num)
    {
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HierarHeapQueue(Pixel* img, double a, double r, int listsize)
{
    ...
    //create hierarchical queue from dhist
    queue = new HierarHeapQueue<Pixel>(dhist, numlevels, nredges, a, listsize, connectivity, r); // +1 for the dummy node
    curSize = 0;
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HierarHeapQueue_Cache(Pixel* img, double a, double r, int listsize)
{
    ...
    //create hierarchical queue from dhist
    queue = new HierarHeapQueue_cache<Pixel>(dhist, numlevels, nredges, a, listsize, connectivity, r); // +1 for the dummy node
    curSize = 0;
    maxSize = 1 + imgsize + dimgsize; //Do not use TSE here, becasue dhist is a logged histogram (also this algorithm is for hdr)
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HierarHeapQueue_Cache_histeq(Pixel* img, int listsize, int a)
{
    ...
    queue = new HierarHeapQueue_HEQ<Pixel>(eqhist, histeqmap, eqhistsize, nredges, coeff, listsize); // +1 for the dummy node
    curSize = 0;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::initialize_node(Pixel *img, Pixel *dimg, Pixel maxpixval)
{
...
void AlphaTree<Pixel>::initialize_node1(Pixel *img, RankItem<double> *rankitem, Pixel maxpixval)
{
    ...
    for (p = 0; p < maxSize;p++)
    {
        ...
        node[p].rootidx = node[p].parentidx = NULLINDEX;
    }
}

template<class Pixel>
void AlphaTree<Pixel>::initialize_node1(Pixel *img, RankItem<double> *rankitem, Pixel maxpixval, _int32* rank2rankitem)
{
    Imgidx p, imgsize = width * height;
    ...
    for (p = 0; p < maxSize;p++)
    {
        ...
        node[q].rootidx = node[q].parentidx = NULLINDEX;
    }
}

template<class Pixel>
void AlphaTree<Pixel>::initialize_node(Pixel *img, RankItem<Pixel> *rankitem, Pixel maxpixval)
{
    Imgidx p, imgsize = width * height;
...
void AlphaTree<Pixel>::initialize_node_par(Pixel *img, RankItem<Pixel> *rankitem, Pixel maxpixval)
{
    ...
    for (p = 0; p < maxSize;p++)
    {
        ...
        {
            node[p].set(1, 0, (double)img[p], img[p], img[p]);
            node[p].parentidx = node[p].rootidx = NULLINDEX;
        }
        else
        {
            node[p].set(0, rankitem[p - imgsize].alpha, 0.0, maxpixval, 0);
...
template<class Pixel>
void AlphaTree<Pixel>::initialize_node_par1(Pixel *img, RankItem<double> *rankitem, Pixel maxpixval, _int32* rank2rankitem)
{
    Imgidx p, imgsize = width * height;
...
template<class Pixel>
void AlphaTree<Pixel>::init_hypergraph_nodes(Pixel *dimg)
{
    if (connectivity == 4)
    {
        Imgidx imgsize = height * width;
...
//easier
template<class Pixel>
void AlphaTree<Pixel>::init_hypergraph_nodes(Imgidx *rank)
{
    if (connectivity == 4)
    {
        Imgidx imgsize = height * width;
...
template<class Pixel>
void AlphaTree<Pixel>::set_isAvailable_hypergraph(_uint8 *isAvailable)
{
    if (connectivity == 4)
    {
        Imgidx dimgidx;
...
template <class Pixel>
void AlphaTree<Pixel>::Flood_Trie_Hypergraph(Pixel *img)
{
...
template<class Pixel>
void AlphaTree<Pixel>::cumsum(Imgidx *hist, Imgidx size, Imgidx &maxidx)
{
    Imgidx sum = hist[0];
    maxidx = 0;
...
template<class Pixel>
void AlphaTree<Pixel>::cumsum(Imgidx *hist, Imgidx size, _uint32 *histeqmap, int eqhistsize)
{
    Imgidx sum = hist[0];
    for (Imgidx i = 1;i < size;i++)
    {
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_HierarQueue_Hypergraph(Pixel* img)
{
    if (sizeof(Pixel) > 2 || channel > 1)
    {
        printf("Error: Hierarchical queues do not work on >16 bits images or multispectral images\n");
...
template<class Pixel>
void AlphaTree<Pixel>::canonicalize(Imgidx nidx)
{
    Imgidx p, q;
...
Imgidx AlphaTree<Pixel>::merge_subtrees(Pixel *dimg, _int64 blksz_x, _int64 blksz_y, Imgidx npartition_x, Imgidx npartition_y, Imgidx* subtree_cur, int tse, Imgidx *nrbnode)
{
    ...
    if (tse)
        ...
        p = 0;
    while(node[p].parentidx != NULLINDEX)
        p = node[p].parentidx;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::merge_subtrees(_uint8 *dimg, _int64 blksz_x, _int64 blksz_y, _int16 npartition_x, _int16 npartition_y, Imgidx* subtree_cur, int tse)
{
    Imgidx numblk;
    _int64 blksz_x0 = blksz_x;
    _int64 blksz_y0 = blksz_y;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::merge_subtrees1(_uint8 *dimg, _int64 blksz_x, _int64 blksz_y, _int16 npartition_x, _int16 npartition_y, Imgidx* subtree_cur, int tse, Imgidx* hypernode_level)
{
    Imgidx numblk;
    _int64 blksz_x0 = blksz_x;
    _int64 blksz_y0 = blksz_y;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::parflood_node_alloc(Imgidx *subtree_size, Imgidx *subtree_start, Imgidx *blkws, Imgidx *blkhs, int numpartitions, double sizemult)
{
    subtree_start[0] = 0;
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_Hierarqueue_par(Pixel *img, int numthreads)
{
    if (sizeof(Pixel) > 2 || channel > 1)
    {
        printf("Error: Hierarchical queues do not work on >16 bits images or multispectral images\n");
        ...
        while (1) //flooding
        {
            while ((_int64)queue->min_level <= (_int64)current_level) //flood all levels below current_level
            {
                ...
                else // To do later
                {
                    ...
                    // if (is_available(isAv, 4) && !isVisited[p - wstride1])	queue->push(p - wstride1, dimg[q - wstride_d + 4]);
                    // if (is_available(isAv, 5) && !isVisited[p - width])   	queue->push(p - width, dimg[q - wstride_d + 1]);
                    // if (is_available(isAv, 6) && !isVisited[p - wstride0])	queue->push(p - wstride0, dimg[q - wstride_d - 2]);
                    // if (is_available(isAv, 7) && !isVisited[p - 1])	      	queue->push(p - 1, dimg[q - 1]);
                }
            ...
            //go to higher level
            iNode = node[stack_top].parentidx;
            if (iNode == NULLINDEX || (_int64)queue->min_level < (_int64)node[iNode].alpha) //new level from queue
            {
                iNode = nidx++;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::find_root(Imgidx p)
{
    if (p == NULLINDEX)
        return NULLINDEX;
    ...
    for (r = p;node[r].rootidx != NULLINDEX;r = node[r].rootidx)
        ;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::find_root_in(Imgidx p)
{
    if (p == NULLINDEX)
        return NULLINDEX;
    ...
    for (r = p;node_in[r].rootidx != NULLINDEX;r = node_in[r].rootidx)
            ;
...
template<class Pixel>
void AlphaTree<Pixel>::Unionfind(Pixel* img)
{
    ...
    for (Imgidx r = 0; r < nredges; r++)
    {
        ...
        if (!unionbyrank || node[x].alpha != pRank->alpha)
        {
            ...
            node[nodeaddr].parentidx = node[nodeaddr].rootidx = NULLINDEX;
...
template<class Pixel>
void AlphaTree<Pixel>::blockwise_tse(Imgidx *subtree_size, Imgidx *subtree_nborderedges, double *nrmsds, Imgidx *dhist, Imgidx *subtree_max, Imgidx *blkws, Imgidx *blkhs, _int8 npartition_x, _int8 npartition_y, Imgidx numbins)
{
    _int64 numpartitions = (_int64)npartition_x * (_int64)npartition_y;
    Pixel maxdiff;
...
template<class Pixel>
void AlphaTree<Pixel>::quantize_ranks_compute_histogram(_uint8 *qrank, Imgidx* rank, Pixel* img, Imgidx *dhist, Imgidx *blkws, Imgidx *blkhs, Imgidx *startpidx, _int64 binsize, Imgidx numbins, _int8 npartition_x, _int8 npartition_y, Imgidx* subtree_max)
{
    _int64 numpartitions = (_int64)npartition_x * (_int64)npartition_y;
...
template<class Pixel>
_uint8 AlphaTree<Pixel>::pow_quantization(Imgidx rank, _uint64 qint)
{
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::descendroots(Imgidx q, _int64 qlevel, AlphaNode<Pixel> *pilottree)
{
...
//Hybrid_Pilot_Rank
template<class Pixel>
void AlphaTree<Pixel>::unionfind_refine_qlevel(_int64 qlevel, _int64 binsize, Imgidx nredges, AlphaNode<Pixel>* pilottree, RankItem<double>* rankitem, _int8* redundant_edge, _int32* rank2rankitem)
{
    ...
    if (qlevel)
    {
        for (Imgidx r = rank_start; r <= rank_end; r++)
        {
            ...
            if ((x!= NULLINDEX) && (x == y)) //already connected, nothing to do
            {
                continue;
            }
...
template<class Pixel>
void AlphaTree<Pixel>::compute_dhist_par(_uint8 *qrank, Imgidx *dhist, Imgidx *startpidx, _int32 numbins, _int8 npartition_x, _int8 npartition_y, _int64 blksz_x, _int64 blksz_y, _int64 blksz_xn, _int64 blksz_yn)
{
    if (connectivity == 4)
    {
...
template<class Pixel>
void AlphaTree<Pixel>::compute_dhist_par_hypergraph(_uint8 *qrank, Imgidx *dhist, Imgidx *startpidx, _int32 numbins, _int8 npartition_x, _int8 npartition_y, _int64 blksz_x, _int64 blksz_y, _int64 blksz_xn, _int64 blksz_yn, Imgidx *blkmaxpidx)
{
    if (connectivity == 4)
    {
        #pragma omp parallel for
...
void AlphaTree<Pixel>::fix_subtreeidx(Imgidx *subtreestart, Imgidx *startpidx, Imgidx *cursizes, _int8 npartition_x, _int8 npartition_y, int numpartitions, _int64 blksz_x, _int64 blksz_y, _int64 blksz_xn, _int64 blksz_yn)
{
    for (int b = 0;b < numpartitions;b++)
    {
        ...
        for (Imgidx p = 0;p < cursizes[b];p++)
            if (ptree[p].parentidx != NULLINDEX)
...
template<class Pixel>
void AlphaTree<Pixel>::merge_subtrees(_uint8 *qrank, Imgidx *qindex, _int64 blksz_x, _int64 blksz_y, Imgidx neighbor_offset, Imgidx shamt, Imgidx npartition_x, Imgidx npartition_y, _int32 numbins)
{
    ...
    //reset rootidxs
    for (p = 0;p < maxSize;p++)
        if (node[p].area) node[p].rootidx = NULLINDEX;
...
template<class Pixel>
void AlphaTree<Pixel>::merge_subtrees(_uint8 *qrank, _int64 blksz_x, _int64 blksz_y, Imgidx neighbor_offset, Imgidx shamt, Imgidx npartition_x, Imgidx npartition_y, _int32 numbins)
{
    Imgidx x, y, r, p, q, dimgidx;
    Imgidx imgsize = height * width;
    ...
    //reset rootidxs
    for (p = 0;p < curSize;p++)
        node[p].rootidx = NULLINDEX;

    //Make sure that the root has the highest
    for (p = parentAry[0];node[p].parentidx != NULLINDEX;p = node[p].parentidx)
        ;
    if (node[p].alpha != (Pixel)(numbins - 1))
    {
        ...
        node[p].parentidx = q;
        node[q].parentidx = node[q].rootidx = NULLINDEX;
    }
}
...
void AlphaTree<Pixel>::connect_pilotnode(AlphaNode<Pixel> *pilottree, Imgidx nredges, Imgidx imgsize)
{
    ...
    for (Imgidx p = 0; p < maxSize; p++)
    {
        ...
        {
            q = parentAry[p];
            if (node[p].parentidx == NULLINDEX && pilottree[q].area == 1)
                node[p].parentidx = pilottree[q].rootidx;
        }
        else
        {
            if (node[p].rootidx == NULLINDEX && node[p].area)
            {
                if (node[p].area == imgsize)
                {
                    if (rootindexcand[omp_get_thread_num()] == NULLINDEX)
                    {
                ...
                q = node[p].parentidx;
                for (r = q;pilottree[r].rootidx == NULLINDEX;r = pilottree[r].parentidx)
                    ;
    ...
    for (int i = 0;i < omp_get_max_threads();i++)
    {
        if (rootidx == NULLINDEX) rootidx = rootindexcand[i];
        else if (rootindexcand[i] != NULLINDEX)
...
template<class Pixel>
void AlphaTree<Pixel>::set_qindex(Imgidx *qindex, Imgidx *dhist, _int64 numpartitions, _int32 numbins, Imgidx npartition_x, Imgidx npartition_y, _int64 blksz_x, _int64 blksz_y, _int64 blksz_xn, _int64 blksz_yn)
{
...
template<class Pixel>
void AlphaTree<Pixel>::set_subtree_root(Imgidx** subtreerootary, Imgidx *strary, Imgidx nonzero_nodeidx_start, Imgidx rootlevel_nodeidx_start)
{
        Imgidx imgsize = width * height;
...
template<class Pixel>
void AlphaTree<Pixel>::find_redundant_nodes(_uint8* is_redundant, Imgidx *rank)
{
    if (connectivity == 4)
    {
        Imgidx imgsize = height * width;
...
template<class Pixel>
void AlphaTree<Pixel>::set_subblock_properties(Imgidx* startpidx, Imgidx *blkws, Imgidx *blkhs, Imgidx *blocksize, _int8 npartition_x, _int8 npartition_y, _int64 blksz_x, _int64 blksz_y, _int64 blksz_xn, _int64 blksz_yn)
{
    Imgidx p = 0, q;
...
template<class Pixel>
void AlphaTree<Pixel>::memalloc_queues(HierarQueue*** queues, _int64 numpartitions, Imgidx* blocksize, Imgidx* subtree_max)
{
    //preallocate hqueues
    *queues = (HierarQueue**)Calloc(numpartitions * sizeof(HierarQueue*));
    for (int blk = 0;blk < numpartitions;blk++)
...
template<class Pixel>
void AlphaTree<Pixel>::compute_dimg_and_rank2index(RankItem<double>*& rankitem, Pixel* img, Imgidx nredges, _int32* rank2rankitem)
{
    if (channel == 1)
    {
        ...
        SortPair<Pixel,Imgidx>* sort_space = (SortPair<Pixel,Imgidx>*)Calloc(2 * nredges * sizeof(SortPair<Pixel,Imgidx>));//new SortPair[2 * N];

        rank_to_index((SortValue<Pixel>*)vals, (Imgidx)nredges, rank2rankitem, 0U, (uint_fast8_t)(sizeof(Pixel) << 3), sort_space, omp_get_max_threads());
    ...
    else
    {
        ...
        SortPair<_uint64,Imgidx>* sort_space = (SortPair<_uint64,Imgidx>*)Calloc(2 * nredges * sizeof(SortPair<_uint64,Imgidx>));//new SortPair[2 * N];

        rank_to_index((SortValue<_uint64>*)vals, (Imgidx)nredges, rank2rankitem, 0U, (uint_fast8_t)(sizeof(double) << 3), sort_space, omp_get_max_threads());
...
template<class Pixel>
void AlphaTree<Pixel>::print_all_trees(AlphaNode<Pixel>* pilottree)
{
...
template<class Pixel>
void AlphaTree<Pixel>::compute_difference_and_sort(Imgidx* rank, RankItem<double>*& rankitem, Pixel* img, Imgidx nredges, _int32*& rank2rankitem)
{
    compute_dimg_and_rank2index(rankitem, img, nredges, rank2rankitem);
...
template<class Pixel>
void AlphaTree<Pixel>::Pilot_Rank(Pixel* img, int numthreads)
{
    Imgidx imgsize, dimgsize, nredges;
        ...
        for (int blk = 0; blk < numpartitions; blk++) //flooding is somehow slower than the one without tse...
        {
            ...
            while (1) //flooding
            {
                while ((_int64)queue->min_level <= (_int64)current_level) //flood all levels below current_level
                {
                    ...
                    //if (stack_top == NULLINDEX) printf("SQEEEEAK\n");
                    ...
                    if (connected_neighbor)
                    {
                        ...
                        //cleanup pawprints
                        for (squirrel = leaf1;node[squirrel].parentidx != squirrel;squirrel = node[squirrel].parentidx)
                            node[squirrel].rootidx = NULLINDEX;
                        node[squirrel].rootidx = NULLINDEX;
                    }
                }
    ...
    {
        rootidx = parentAry[0];
        while(node[rootidx].parentidx != NULLINDEX)
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::NewAlphaNode(Imgidx &size, Imgidx &maxsize)
{
    if (size == maxsize)
    {
//std::cout << "Reallocating...\n";
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::NewAlphaNode(AlphaNode<Pixel> *tree, Imgidx &size, Imgidx &maxsize, Pixel level, AlphaNode<Pixel> *pCopy)
{
...
template<class Pixel>
void AlphaTree<Pixel>::remove_redundant_node(AlphaNode<Pixel> *tree, Imgidx &size, Imgidx& prev_top, Imgidx& stack_top)
{
...
//		if (!path_compression)
//		{
//		while((q = node[p].parentidx) != NULLINDEX)
//			p = q;
//			return p;
//		}
//		else
    {
        r = p;
        while((q = node[p].rootidx) != NULLINDEX)
            p = q;
...
template<class Pixel>
void AlphaTree<Pixel>::Flood_Trie(Pixel* img)
{
    Imgidx imgsize, dimgsize, nredges;
    Imgidx current_rank = 0, next_rank = 0;
    RankItem<double> *rankitem, *pRank;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::get_level_root(Imgidx p, Imgidx nodeidx)
{
    while(node[p].parentidx != NULLINDEX && nodeidx > node[p].parentidx)
        p = node[p].parentidx;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::get_level_root(Imgidx p)
{
    return get_level_root(p, node);
}

template<class Pixel>
Imgidx AlphaTree<Pixel>::get_level_root(Imgidx p, Pixel alpha)
{
    while(node[p].parentidx != NULLINDEX && alpha >= node[node[p].parentidx].alpha)
        p = node[p].parentidx;
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::get_level_root(Imgidx p, AlphaNode<Pixel> *tree)
{
    if (p == NULLINDEX)
        return NULLINDEX;
    ...
    while(1)
    {
        ...
        if (parent == NULLINDEX || tree[parent].alpha > a)
            break;
        p = parent;
        //cntcnt++;
    }
...
template<class Pixel>
void AlphaTree<Pixel>::swap(Imgidx& x, Imgidx& y)
{
    Imgidx tmp = x;
    x = y;
    y = tmp;
...
template<class Pixel>
void AlphaTree<Pixel>::swap(AlphaNode<Pixel>** x, AlphaNode<Pixel>** y)
{
...
template<class Pixel>
Imgidx AlphaTree<Pixel>::get_nearest_common_ancestor(Imgidx x, Imgidx y)
{
    Imgidx p;
    for (p = y;p != NULLINDEX;p = node[p].parentidx)
        node[p].rootidx = NULLINDEX;
    for (p = x;p != NULLINDEX;p = node[p].parentidx)
        node[p].rootidx = x;
    for (p = y;p != NULLINDEX && node[p].rootidx != x;p = node[p].parentidx)
        ;
    return p;
}

template<class Pixel>
Pixel AlphaTree<Pixel>::get_nearest_common_ancestor_level(Imgidx x, Imgidx y)
{
    Imgidx p;
...
template<class Pixel>
Pixel AlphaTree<Pixel>::connect(Imgidx x, Imgidx y, Imgidx newidx, Pixel alpha)
{
    Imgidx x0, y0, z;
//		bool compxy;
    Imgidx imgsize = height * width;
//		Imgidx x1 = x, y1 = x;
    ...
    node[newidx].parentidx = NULLINDEX;
    ...
    if (x == NULLINDEX ||
        (y != NULLINDEX && ((node[x].alpha > node[y].alpha) ||
    ...
    while(1)
    {
        if (y == NULLINDEX)
        {
            while(node[x].parentidx != NULLINDEX)
            {
                x = (node[x].parentidx);
                node[x].add(q);
                if (node[x].area == imgsize)
                {
                    node[x].parentidx = NULLINDEX;
                    break;
                }
            }
            break;
        ...
        while(1)
        {
            ...
            if (x0 != NULLINDEX && (node[x0].alpha < node[y].alpha))
            {
                x = x0;
...
template<class Pixel>
Pixel AlphaTree<Pixel>::connect(Imgidx x, Imgidx y, Pixel alpha, Imgidx newidx)
{
    Imgidx x0, y0, z;
//		bool compxy;
    Imgidx imgsize = height * width;
//		Imgidx x1 = x, y1 = x;
    ...
    if (x == NULLINDEX || (y != NULLINDEX && (x > y)))
    {
        q->copy(node + x0);
        swap(x,y);
    }
...
template<class Pixel>
void AlphaTree<Pixel>::canonicalize()
{
    Imgidx p;
    Imgidx numcan = 0;
    Imgidx imgsize = height * width;
    ...
    for (p = 0;p < imgsize;p++)
    {
        ...
        if (r != NULLINDEX && node[q].alpha == node[node[q].parentidx].alpha)
        {
    ...
    {
        if (node[p].area && node[p].parentidx != NULLINDEX && node[node[p].parentidx].parentidx != NULLINDEX &&
    ...
    for (p = maxSize;p < maxSize;p++)
    {
        ...
        if (node[q].area == 0 && (node[p].parentidx != NULLINDEX || node[p].rootidx != NULLINDEX))
        {
            node[q].parentidx = node[q].rootidx = NULLINDEX;
        }
...
template<class Pixel>
void AlphaTree<Pixel>::merge_subtrees(Imgidx *rank, RankItem<Pixel>* rankitem, _int64 blksz_x, _int64 blksz_y, Imgidx neighbor_offset, Imgidx shamt, Imgidx npartition_x, Imgidx npartition_y)
{
...
template<class Pixel>
void AlphaTree<Pixel>::set_subimgsizes(Imgidx** subimgsizes, _int8 npartition_x, _int8 npartition_y, _int64 blksz, _int64 blksz_lastcol, _int64 blksz_lastrow, _int64 blksz_last)
{
